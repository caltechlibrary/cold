...
<section class="thesis-search-results" id="thesis-search-results"></section>`;
    ...
    private populateDatalist(id: string, values: string[]): void {
        ...
        if (!dl) return;
        dl.innerHTML = values.map((v) => `<option value="${escapeHTML(v)}">`)
            .join(
                "",
            );
    }
    ...
    private updateURL(): void {
        ...
        const order =
            (document.getElementById("order") as HTMLSelectElement).value;
        p.set("order", order);
    ...
    private async runSearch(): Promise<void> {
        ...
        const satisfyAll =
            (document.getElementById("satisfyall") as HTMLSelectElement)
                .value ===
                "ALL";
        ...
        const eprintidVal =
            (document.getElementById("q_eprintid") as HTMLInputElement).value
                .trim();
        if (eprintidVal) {
            conditions.push(() =>
                this.fetchResults("by_eprintid", eprintidVal)
            );
        }

        // Year range
        const yearFrom =
            (document.getElementById("year_from") as HTMLInputElement)
                .value.trim();
    ...
    private renderResults(records: ThesisRecord[]): void {
        ...
        this.resultSection.innerHTML =
            `<p>${sorted.length} record(s) found.</p>`;
    ...
    private formatTable(records: ThesisRecord[]): string {
        const rows = records.map((r) => {
            const authors = (r.creators ?? []).map((c) =>
                `${c.family}, ${c.given}`
            )
                .join("; ");
            ...
            const divs = (r.divisions ?? []).map((d) => divisionLabels[d] ?? d)
                .join(
                    "; ",
                );
            const opts = (r.option_major ?? []).map((o) => optionLabels[o] ?? o)
                .join(
                    "; ",
                );
