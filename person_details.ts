// renderPersonDetails builds the HTML fragment shown in the person lookup
// panels (e.g. people_rename.ts's "old_clpid" preview). It is pure/DOM-free
// so it can be imported directly by tests without pulling in a module that
// wires up `document.addEventListener` at load time.
export function renderPersonDetails(p: Record<string, unknown>): string {
    const parts: string[] = [];
    ...
    if (p.thesis_id) {
        parts.push(
            `<div class="thesis_id">CaltechTHESIS ID: ${p.thesis_id}</div>`,
        );
    }
    if (p.advisors_id) {
        parts.push(
    ...
    if (p.internal_notes) {
        parts.push(
            `<div class="internal-notes"><strong>Internal Notes:</strong><pre>${
                String(p.internal_notes).replace(/&/g, "&amp;").replace(
                    /</g,
                    "&lt;",
                )
                    .replace(/>/g, "&gt;")
