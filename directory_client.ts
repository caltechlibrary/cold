...
function updateRecord(obj: object) {
    const family_name_elem: HTMLElement = document.getElementById(
            "family_name",
        ),
        lived_name_elem: HTMLElement = document.getElementById("given_name"),
        display_name_elem: HTMLElement = document.getElementById(
            "display_name",
        ),
        ...
        if (resp.ok) {
            ...
            if (resp.status === 404) {
                ...
                if (bio_status_elem !== null) {
                    bio_status_elem.textContent =
                        "⚠ Directory ID not found — bio may be edited manually.";
                }
            }
        }
    }
