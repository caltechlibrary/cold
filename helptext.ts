...
{app_name} CLPID [--record_ids] [--format=csv|xlsx]
...
{app_name} [OPTIONS] [CLPID] [ORCID]

# DESCRIPTION

{app_name} retrieves publication records from the Invenio RDM JSON API for CaltechAUTHORS
...
~~~shell
{app_name} -f json "" 0000-0000-0000-0000
~~~

Retrieve publications as CSV:

~~~shell
{app_name} -f csv Newman-D-K
~~~

Retrieve publications using both CLPID and ORCID:

~~~shell
{app_name} Newman-D-K 0000-0000-0000-0000
~~~

Retrieve publications with unknown CLPID:

~~~shell
{app_name} "" 0000-0000-0000-0000
~~~
...
{app_name} [OPTIONS] COUNTRY_CODE

# DESCRIPTION

{app_name} generates a country collaboration report for CaltechAUTHORS records.
...
export const generateTechnicalReportsRptHelpText =
  `%{app_name}(1) user manual | {version} {release_hash}
% R. S. Doiel
% {release_date}
...
{app_name} generates the technical reports report for CaltechAUTHORS. It takes
...
~~~shell
{app_name} >technical_reports.csv
~~~

Count the records in the report:

~~~shell
{app_name} | csv2json | jsonrange -length
~~~
