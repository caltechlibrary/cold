...
func main() {
	...
	for _, voc := range vocs {
		fmt.Fprintf(out, `
-- %s %s
INSERT INTO cold.vocabulary_description (namespace, description) VALUES ('%s', '%s');`, voc.Name, voc.Description, voc.Name, voc.Description)
		for k, v := range voc.Dict {
			fmt.Fprintf(out, `INSERT INTO cold.vocabulary (namespace, key, value) VALUES ('%s', '%s', '%s');
`, voc.Name, k, v)
		}
		fmt.Fprintln(out, "")
	}
