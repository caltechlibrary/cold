// collaborator_affiliations_report.ts
var CollaboratorAffiliationsReportUI = class {
  cName = "people.ds";
  reportElement;
  clpidElement;
  baseUrl;
...
`;
  ...
  async setupReport(clpid) {
    ...
    if (!await this.isValidClpid(clpid)) {
      this.resultSection.innerText = `Invalid clpid '${clpid}', enter clpid and press \u2699\uFE0F`;
      return;
    }
    let resp = await this.postReportRequest("run_collaborator_affiliations_report", clpid, "");
    if (resp.ok) {
      const link = document.createElement("a");
      link.href = "./reports";
      link.title = "return to reports page for pickup";
      link.textContent = "queued";
      this.resultSection.replaceChildren(document.createTextNode(`The collaborator affiliations report for "${clpid}" is `), link);
    } else {
      this.resultSection.innerText = `There was a problem submitting the collaborator affiliations report for "${clpid}", ${JSON.stringify(resp)}`;
    }
  }
};
export {
  CollaboratorAffiliationsReportUI
};
