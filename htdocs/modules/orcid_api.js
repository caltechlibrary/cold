...
export async function getORCIDNames(orcid) {
  ...
  if (resp.ok) {
    ...
    if (rec.person.name["given-names"] !== undefined && rec.person.name["given-names"].value !== undefined) {
      given_name = rec.person.name["given-names"].value.trim();
    }
    if (rec.person.name["family-name"] !== undefined && rec.person.name["family-name"].value !== undefined) {
      family_name = rec.person.name["family-name"].value;
    }
    if (rec.person.name["credit-name"] !== undefined && rec.person.name["credit-name"].value !== undefined) {
