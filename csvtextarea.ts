...
class CSVTextarea extends HTMLElement {
  ...
  async initializeComponent() {
    ...
    template.innerHTML = `
      ...
      ${
      this.hasAttribute("title") || this.hasAttribute("help-description")
  ...
  initializeTable() {
    ...
    headings.forEach((heading, index) => {
      const datalist = this.shadowRoot?.querySelector(
        `datalist#${heading.toLowerCase()}`,
      ) as HTMLDataListElement | null;
      if (datalist) {
        this.setAutocomplete(
          index,
          Array.from(datalist.options).map((option: HTMLOptionElement) => ({
            value: option.value,
          })),
        );
      }
  ...
  setupEventListeners() {
    // Append Row
    this.shadowRoot?.querySelector("#append-row")?.addEventListener(
      "click",
      () => this.appendRow(),
    );

    // Cleanup
    this.shadowRoot?.querySelector("#cleanup")?.addEventListener(
      "click",
      () => this.cleanupTable(),
    );
    ...
    if (this.hasAttribute("debug")) {
      this.shadowRoot?.querySelector("#debug")?.addEventListener(
        "click",
        () => {
    ...
    // Input Event
    this.shadowRoot?.querySelector("tbody")?.addEventListener(
      "input",
      (event) => {
        ...
        if (target.tagName === "INPUT") {
          ...
          if (row && cell) {
            ...
            if (this.hasAttribute("debug")) {
              console.log(
                `Cell changed: Row ${rowIndex}, Col ${colIndex}, Value ${value}`,
              );
            }
          }
        }
      ...
      (event) => {
        ...
        if (target.tagName === "INPUT") {
          ...
          if (row && cell) {
            ...
            if (this.hasAttribute("debug")) {
              console.log(
                `Cell focused: Row ${rowIndex}, Col ${colIndex}, Value ${value}`,
              );
            }
          }
        }
    ...
    // Sync textarea before the form serialises it for submission
      ...
      (event) => {
        ...
        if (event.key === "Backspace" && target.tagName === "INPUT") {
          ...
          if (start !== null && end !== null && start === end && start > 0) {
            input.value = input.value.slice(0, start - 1) +
  ...
  cleanupTable() {
    ...
    if (tbody) {
      ...
      for (let i = rows.length - 1; i >= 0; i--) {
        const shouldDelete = this.isEmptyRow(i) ||
          (this.customCleanupFilter !== undefined &&
  ...
  fromObjects(
    objects: Array<{ rowIndex: number; colIndex: number; value: string }>,
  ) {
    const tbody = this.shadowRoot?.querySelector("tbody");
  ...
  getCellValue(rowIndex: number, colIndexOrName: number | string): string {
    const colIndex = typeof colIndexOrName === "number"
      ? colIndexOrName
      : this.getColumnIndexByName(colIndexOrName);
    const cell = this.shadowRoot?.querySelector(
      `tbody tr:nth-child(${rowIndex + 1}) td:nth-child(${colIndex + 1}) input`,
  ...
  setAutocomplete(
    ...
    const inputs = this.shadowRoot?.querySelectorAll(
      `tbody td:nth-child(${colIndex + 1}) input`,
    );
  ...
  getAutocomplete(colIndexOrName: number | string): string[] | undefined {
    const colIndex = typeof colIndexOrName === "number"
      ...
      : this.getColumnIndexByName(colIndexOrName);
    const datalist = this.shadowRoot?.querySelector(
      `datalist#column-${colIndex}`,
    );

    if (datalist) {
      const options: string[] = [];
      datalist.querySelectorAll("option").forEach(
